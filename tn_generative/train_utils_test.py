...
class RunTrainingTests(absltest.TestCase):
  ...
  def setUp(self): 
    ...
    self.experiment_config.results.save_results = False
    self.experiment_config.job_id = 1
    self.experiment_config.task_id = 0
    self.experiment_config.data.dir = os.path.join(current_dir, 'test_data')
    self.experiment_config.data.filename = 'example_dataset.nc'
